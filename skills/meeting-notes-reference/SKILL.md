...
## Audio Input Pipeline（音檔輸入流程）

使用者提供的是**音檔或錄影**（mp3 / m4a / wav / mp4 等）而非文字時，先走三段式管線，
不要嘗試自己「聽」音檔，也不要用 markitdown 轉錄（中文會議品質差）：

### 1. 轉錄段 — Breeze-ASR-25（台灣國語＋中英夾雜特化）

```bash
python scripts/transcribe.py <音檔路徑> [--srt]
```

- 腳本會自動：ffmpeg 前處理（16kHz 單聲道＋響度正規化）→ faster-whisper CPU int8 推論
  → 輸出 `<音檔名>.transcript.txt`（含時間戳）。
- 術語偏置自動讀取 `references/glossary.md`。
- 依賴：`ffmpeg`、`pip install faster-whisper`；首次執行自動從 HuggingFace
  下載模型 `SoybeanMilk/faster-whisper-Breeze-ASR-25`（約 3GB）。缺依賴時
  先告知使用者安裝指令，不要默默改用其他轉錄方式。
- CPU 推論速度有限，超過 30 分鐘的會議先告知使用者需要等待，考慮用
  `run_in_background` 執行。

### 2. 校正段 — 依術語表修同音錯字

轉錄完成後，讀 `references/glossary.md`，依「校正規則」逐段檢查逐字稿：
修正表列的同音／近音錯字；不確定的保留原文加（?）；數字與日期矛盾時標注待確認。
發現表上沒有的新錯誤模式時，建議使用者補進術語表（經確認後才寫入）。

### 3. 整理段 — 走以下既有流程

校正後的逐字稿即為 Required Inputs 的 transcript，依會議類型套模板整理。
